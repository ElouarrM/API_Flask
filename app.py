...
def extract_full_name_from_pdf(filename):
    ...
    filename_without_cv_resume = filename_without_extension.replace("cv", "").replace("resume", "").replace("CV","").replace("RESUME","").replace("Cv","").replace("Resume","")
    
    # Supprimer les occurrences de ".pdf" ou d'autres extensions
    filename_without_extension_or_extension = filename_without_cv_resume.split(".")[0]
